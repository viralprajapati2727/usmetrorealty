...
abstract class modSPPortfolioJHelper
{
	...
	public static function getList($params, $start, $limit){
		...
		//echo $locstate; exit;
		$stateArr = array("Arizona","Oregon");
		$locstateNew = in_array($locstate, $stateArr);
		if(empty($locstateNew)){
			$locstateNewAZ = "Arizona";
          	$locstateNewOR = "Oregon";
		} else {
			$locstateNew = $locstateNew;
		}
		...
		//echo $locstate;exit;
		 $where  = array();
		 $where['locstate']		= $locstateNewAZ;
         //$where['locstate']		= $locstateNewOR;
        ...
        //$query->setLimit(12);
        //echo $query; exit;
        $db->setQuery($query, $_GET['start'], $count);
