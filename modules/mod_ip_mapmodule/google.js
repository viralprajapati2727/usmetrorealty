...
var ipMapModFunctions = (function(app) {
    ...
    **************/
	...
	if(listing.cat_id == 1){ dwellingType = 'Residential'; }
	if(listing.cat_id == 2){ dwellingType = 'Residential Rental'; }
	if(listing.cat_id == 3){ dwellingType = 'Land and Lots'; }
	if(listing.cat_id == 4){ dwellingType = 'Comm/Industry Sale'; }
	if(listing.cat_id == 5){ dwellingType = 'Comm/Industry Lease'; }
	if(listing.cat_id == 6){ dwellingType = 'Multiple Dwellings'; }
	
	//if(listing.formattedprice == 'COM_IPROPERTY_CALL_FOR_PRICE'){ listing.formattedprice = 'Call for price'; }
	var listingPrice2 = '';
	if(listing.call_for_price == 0){
		if(listing.price2 == 0){
			listingPrice2 = listing.formattedprice;
		} else {
			listingPrice2 = '$'+listing.price2;
		}
	}else {
		listingPrice2 = listing.formattedprice;	
       ...
       /* var contentString = '<div class="row-fluid ip-bubble-window">' +
                                ...
                                '</div>' +
                            '</div>';*/
		if((listing.cat_id == 1) || (listing.cat_id == 2)){					
							
			 var contentString ='<div class="row-fluid ip-bubble-window wtplmap">' +
						'<div class="ip-overview-img 123 webi-mapbox"><div class="ip-property-thumb-holder"><div class="span6"><a href="'+listing.proplink+'">'+listing.thumb+'</a></div><div class="span6"><span><strong>'
						+dwellingType+' </strong></span><strong>Price: </strong><strong>'+listingPrice2+
		...
		if((listing.cat_id == 3) || (listing.cat_id == 4) || (listing.cat_id == 5) || (listing.cat_id == 6)){					
							
			 var contentString ='<div class="row-fluid ip-bubble-window wtplmap">' +
						'<div class="ip-overview-img 123 webi-mapbox"><div class="ip-property-thumb-holder"><div class="span6"><a href="'+listing.proplink+'">'+listing.thumb+'</a></div><div class="span6"><strong>'
						+dwellingType+' </strong></span><strong>Price: </strong><strong>'+listingPrice2+
						' </strong><br/><strong>Sqft: </strong><span> '+listing.sqft
						+'</span><br/><strong>Address: </strong><span> '+listing.street_address+' '+listing.statename+'</span><br/></div></div></div>';
