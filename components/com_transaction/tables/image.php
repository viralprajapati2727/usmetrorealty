...
class TransactionTableImage extends JTable
{
	public function __construct(&$_db)
	{
		parent::__construct('#__transaction_images', 'id', $_db);
	}
	...
	public function check()
	{
		...
		if (empty($this->ordering)) {
			...
			$this->ordering = self::getNextOrder('`transaction_id` = '.$this->_db->Quote($this->transaction_id));
		}
	...
	public function store($updateNulls = false)
	{
		...
		{
			// Get the old row
			$oldrow = JTable::getInstance('Image', 'TransactionTable');
			...
			if ($oldrow->state >= 0){
				// Reorder the oldrow
				$this->reorder('`transaction_id`=' . $this->_db->Quote($this->transaction_id).' AND state>=0');
			}
		}
	...
	public function publish($pks = null, $state = 1, $userID = 0)
	{
		...
		$table = JTable::getInstance('Image','TransactionTable');
        ...
        foreach($pks as $pk){
            ...
            {			
                ...
                if (!$table->remote){
                    ...
                    $query->select("id");
                    $query->from("#__transaction_images");
