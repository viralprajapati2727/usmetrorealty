...
class TransactionControllerAjax extends JControllerLegacy
{
    ...
    public function ajaxGetListing()
    {
        ...
        $jinput         = JFactory::getApplication()->input;
        $transaction_id = $jinput->get('id', false, 'INT');
        
        $model = $this->getModel('property');
        
        if ($transaction_id){     
            try {
    			$transaction = $model->getItem($transaction_id);
    			echo new JResponseJson($transaction);
    		} catch (Exception $e){
    			echo new JResponseJson($e);
            }
        } else {
            echo new JResponseJson('No transaction_id found');
        }
    }
    ...
    public function checkTransactionMLS()
    {
      //  JSession::checkToken() or die( 'Invalid Token');

        $app        = JFactory::getApplication();
        $MLS      = $app->input->getString('MLS');
        //$agent_id   = $app->input->getInt('agent_id', 0);
        if(!$MLS) return false;
    ...
    public function ajaxLoadGallery()
    {
        ...
        */
        ...
        if (!$transaction_id ){
            echo json_encode(ipropertyHTML::createReturnObject('error', JText::_('NO TRANSACTION ID INCLUDED')));
            die();
        }
    ...
    public function ajaxAutocomplete()
    {
        ...
        */
        ...
        if($valid){
            ...
            $query->where('p.id IN (SELECT pm.transaction_id FROM #__iproperty_propmid pm WHERE pm.cat_id IN (SELECT id FROM #__iproperty_categories c WHERE c.state = 1 AND (c.publish_up = '.$nullDate.' OR c.publish_up <= '.$nowDate.') AND (c.publish_down = '.$nullDate.' OR c.publish_down >= '.$nowDate.') AND c.access IN ('.implode(",", $groups).')))');
    ...
    {
		if (!$transaction_id = JRequest::getString('id')) {
			echo new JResponseJson('','NO TRANSACTION ID', true);
			die();
		}
		$db		= JFactory::getDbo();
		...
		$query->select($db->quoteName('id'))
			...
			->where($db->quoteName('mls_id') . ' = '. $db->quote($transaction_id));
		$db->setQuery($query);
		$pid = $db->loadResult();
