...
$map_script = "    
        var ipGalleryOptions = {
            ...
            client: '".$app->getName()."',
            allowedFileTypes: [{title : 'Files', extensions : 'jpg,gif,png,pdf,doc,txt,jpeg,mp4,doc,dot,wbk,docx,docm,dotx,dotm,docb,xls,xlt,xlm,xlsx,xlsm,xltx,xltm,xlsb,xla,xlam,xll,xlw,ppt,pot,pps,pptx,pptm,potx,potm,ppam,ppsx,ppsm,sldx,sldm'}]
        };";
...
<script type="text/javascript">
  jQuery(document).ready(function(){
    //jQuery("#jform_listing_price").formatCurrency();

    jQuery("#jform_listing_price").blur(function () {
        jQuery("#jform_listing_price").formatCurrency();
    });
    jQuery("#jform_sold_price").blur(function () {
        jQuery("#jform_sold_price").formatCurrency();
    });
});
        ...
        jQuery('.icon-pencil').click(function(){
                   // jQuery('#custom_modal_backdrop').remove();
                    jQuery('div#ip_image_form').removeClass('hide');
           			jQuery('div#ip_image_form').css('margin-left', '25%');
        });
        ...
        jQuery('.icon-minus').click(function(){
            setTimeout(function(){
             ...
             jQuery(".modal").css('z-index', 9999);
              jQuery('.bootbox').removeClass('fade');
              jQuery('.bootbox').css('margin-left', '25%');
...
<style type="text/css">
    ...
    #upload_files .tab-content { float: left; width: 100%; }
</style>
            ...
            <div class="tab-content" style="padding-top: 20px">
                ...
                <div role="tabpanel" class="tab-pane" id="propertyaddress">
                   <div class="control-group">
                        ...
                        <input type="text" size="50" class="inputbox" value="<?php echo $data->address; ?>" id="jform_address" name="jform[address]" >
                    </div>
                    <div class="control-group">
...
jQuery(document).click(function(){
    ...
    jQuery(document).on('click', "#upload_button", function(e){
        ...
        if(jQuery('#jform_youtube_url').val() !== ''){
            var transaction_id = '<?php echo $transaction_id;?>';
              ...
              if(form_data !== ''){
                var url = "index.php?option=com_transaction&view=transaction&task=transaction.uploadVideo";
                jQuery.ajax({
                    type: "POST",
                    url: url,
                    data: form_data,
    ...
    jQuery(document).on('click', ".delete_video", function(e){
        ...
        //alert(delete_value);
        var url = "index.php?option=com_transaction&view=transaction&task=transaction.deleteVideo";
                jQuery.ajax({
                    type: "POST",
                    url: url,
