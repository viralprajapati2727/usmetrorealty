...
class ExtraWatchJoomlaEnv implements ExtraWatchEnv  	 	    	    		  	 	  	 	  		 	 		    	 			 	   		  	 	 		 	 	   	      	  	 		 		 				 			 		  		    	 		 		  
{
    ...
    function sendMail($recipient, $sender, $recipient2, $subject, $body, $true, $cc, $bcc, $attachment, $replyto, $replytoname)  	 	    	    		  	 	  	 	  		 	 		    	 			 	   		  	 	 		 	 	   	      	  	 		 		 				 			 		  		    	 		 		  
  {
        ...
        if (version_compare(JVERSION, "1.6.0", "ge")) {  	 	    	    		  	 	  	 	  		 	 		    	 			 	   		  	 	 		 	 	   	      	  	 		 		 				 			 		  		    	 		 		  
            ...
            return JUtility::sendMail("$recipient2", "$sender", "$recipient2", "$subject", "$body", 1);  	 	    	    		  	 	  	 	  		 	 		    	 			 	   		  	 	 		 	 	   	      	  	 		 		 				 			 		  		    	 		 		  
        }
  ...
  function getAgentNotPublishedMsg($database) {  	 	    	    		  	 	  	 	  		 	 		    	 			 	   		  	 	 		 	 	   	      	  	 		 		 				 			 		  		    	 		 		  
    ...
    public function getUsernameById($userId) {
        $user = $this->checkIfUserExistsAndGetUser($userId);
        if (@$user) {
            return @$user->username;
        }
        return false;
    ...
    public function getExtraWatchControllerUrl() {
        if (version_compare(JVERSION, '3.2.0', 'ge')) {
            return "?option=com_ajax&module=extrawatch_agent&format=raw&";
        } else {
            return "index.php?option=com_extrawatch&";
        }
    }
    ...
    /**
     * This method exists, because Joomla own JFactory::getUser adds stupid warning on screen
     * @param $userId
     * @return bool|JUser
     */
    private function checkIfUserExistsAndGetUser($userId) {
        $db = JFactory::getDbo();
        $query = sprintf("select count(id) as `count` from #__users where id = '%d'", (int) $userId);
        $db->setQuery($query);
    ...
    public function getCMSFileSystemRootPath() {
		return str_replace("administrator","",JPATH_BASE);
    }
