...
//print_r($where);
        ...
        {
            case 'advsearch':
                $select_string = 'p.id AS id, p.title AS title, p.show_map, p.yearbuilt, p.street_num, p.street, p.street2, p.apt, p.city, p.locstate, p.province, p.region, p.country, p.short_description as short_description, p.mls_id, p.stype_freq, p.latitude, p.longitude, p.price, p.price2, p.beds, p.baths, p.sqft, p.call_for_price, p.created, p.modified, p.available, p.expired, p.stype, p.hide_address, LEFT(p.description,300) as description, p.alias as alias, p.publish_up, p.modified';
            break;
            case 'advsearch2':
            ...
            break;
            case 'properties':
                $select_string = 'p.id AS id, p.mls_id, p.yearbuilt, p.price, p.call_for_price, p.show_map, p.hide_address, CAST(p.price2 AS SIGNED) as price2, p.stype_freq, p.title AS title, p.beds, p.baths, p.sqft, p.lot_acres, p.lotsize, p.street_num, p.street, p.street2, p.apt, p.description as description, p.short_description, p.created, p.created_by, p.modified, p.stype, p.listing_office, p.city, p.locstate, p.county, p.region, p.subdivision, p.country, p.province, p.alias as alias, p.featured, p.latitude, p.longitude, p.available, p.expired, p.income, p.tax, p.publish_up, p.state, p.checked_out, p.checked_out_time, p.hits, p.listing_info, (SELECT DISTINCT cat_id FROM #__iproperty_propmid as pm WHERE pm.prop_id = p.id LIMIT 1) AS cat_id';
                // $joins[] = '#__iproperty_propmid as pm ON pm.prop_id = p.id';
                // $query[] = 'pm.cat_id != 0';
            break;
            case 'ipuser':
        ...
        //echo "select_string="; echo $select_string;echo "--";
        ...
        if (isset($where['property'])){
            foreach ($where['property'] as $field => $value){
                ...
                //echo "field="; echo $field;echo "--";
                
                if ($field == 'expired'){
                    $today_date = date('Y-m-d');
                    //$query[] = $this->_db->quoteName('p.expired').' > '.$today_date;
                    $query[] = '(STR_TO_DATE('.$this->_db->quoteName('p.expired').',"%Y-%m-%d") > STR_TO_DATE("'.$today_date.'","%Y-%m-%d") OR '.$this->_db->quoteName('p.expired').' IS NULL)';
                }

                if ($field == 'keyword' && $value){
                    $searchwheres = self::textSearch($value, $where['searchfields']);
                    //echo ' searchwheres=';print_r($searchwheres);
                ...
                } else if ($field == 'subdivision' && $value){
                    //echo "field=subdivision-value="; echo $value;echo "--";
                    // this does not work---need to add it the result set or something to check for  flag to display new text****'COM_IPROPERTY_PROPERTIES'="Subdivision Properties";
                    $query[] = $this->_db->quoteName('p.'.$field).' LIKE ('.JString::strtolower($this->_db->Quote('%'.trim($value).'%', true)).')';
                } else {
                    //echo "field=else-value="; echo $value;echo "--";
                    $fieldvar = '';
                    if(is_array($value) && !empty($value)){
                        foreach ($value as $v){
        ...
        if (isset($where['categories'])) {
            ...
            if($checked){
                ...
                $pmquery .= ' AND (SELECT DISTINCT cat_id FROM #__iproperty_propmid as pm WHERE pm.prop_id = p.id LIMIT 1) IN ('.rtrim($checked, ',').') ';
                // echo $pmquery; exit;
            }
        }
        ...
        if (isset($where['locstate'])){     
                    $db = JFactory::getDbo();

                    $query2 = $db->getQuery(true);
                    $query2->select('id, title')
                        ->from('#__iproperty_states')
                        ->where('title = '.$db->quote($where['locstate']));

                    $db->setQuery($query2, 0, 1);
                    $result = $db->loadObject();
                    $res = count($result);
                    if($res == 1){
                         $query[] = 'p.locstate = '.$result->id;
                    }
        ...
        if (isset($where['geopoint'])){
            if(isset($where['geopoint']['lat']) && isset($where['geopoint']['lon']) && isset($where['geopoint']['rad'])){
                ...
                // (c) http://www.movable-type.cecho "<pre>"; print_r($joins); exit;echo "<pre>"; print_r($joins); exit;echo "<pre>"; print_r($joins); exit;o.uk/scripts/latlong-db.html
                // rad must be in KM!!
        ...
        $joins1[1] = '#__iproperty_countries as cn ON cn.id = p.country';
        $joins1[2] = '#__iproperty_states as st ON st.id = p.locstate';
        $joins[3] = '#__iproperty_cities as ct ON ct.id = p.city';
        $joins = array_unique($joins);
        //echo "<pre>"; print_r($joins); exit;
        ...
        if (is_array($joins1) && count($joins1)){
            foreach ($joins1 as $join1){
                $query->join('INNER', $join1);
            }
        }
        $query->where($where);
         //echo $query; exit;
        if ($this->_sort && !$this->_order) // this means random sort (ie featured display)
        {
            ...
            if($this->_sort == 'p.price2'){
               $this->_sort = 'price2'; 
            }
            $query->order($this->_sort .' '. $this->_order);
            //$query = CAST(p.price2 AS SIGNED)) as p.price2;
           // echo $query; exit;
        }
        ...
        //echo str_replace("#_", "c4aqr", $query); exit;
        //echo "query="; echo $query;echo "--"; exit;
        return $query;
    }
    ...
    private static function textSearch($keyword, $searchfields = false)
    {
        ...
        //print_r($keyword_array); exit;
        $against        = '';
		$settings   	= ipropertyAdmin::config();
                //echo 'search fields init:';
                //print_r($searchfields);
        if (!$searchfields){
            //kalim added the subdivision on 12-27-18 - and locstate
            $searchfields = array('title','street_num','street','street2','city','county','region','postcode','short_description','description','mls_id','subdivision','locstate');
        }
        
        if ($settings->match_against) {
			// new MATCH / AGAINST syntax
            $keyword_array = str_replace(" ", "", $keyword);
            //print_r("<pre>"); print_r($keyword_array); //exit; //------------
		foreach ($keyword_array as $keyword){
               ...
               if(empty($keyword)){
                ...
                //print_r("<keyword_search>"); print_r($keyword_search);//---------
               }
				
		}
            // custom add
            $keyword_search[] = 'MATCH ('.implode(',', $searchfields).') AGAINST ('.$db->Quote('+'.trim($keyword_array).'*').' IN BOOLEAN MODE) AND';
			$searchterm = implode(' ', $keyword_search);

                //print_r("<searchterm >"); print_r($searchterm ); exit;//---------
    ...
    public static function buildAgentsProfileQuery( $db, $where, $sort = false, $order = 'ASC', $debug = false, $with_listings = false )
    {
        $query = $db->getQuery(true);
