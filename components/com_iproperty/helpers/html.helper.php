...
public static function getrecentThumbnail($prop_id, $link = '', $alt = '', $width = '', $imgattributes = '', $linkattributes = '', $suffix = true, $tags = true, $rel_path = true)
    {
        $db             = JFactory::getDbo();
        $settings       = ipropertyAdmin::config();        
        ...
        if ($tags || empty($tags)){
            //create thumbnail image with link if applicable
            $thumbimg = '';
            $thumbimg .= '<a href="index.php?option=com_iproperty&view=property&id='.$prop_id.'&Itemid=323">';
            $thumbimg .= '<img src="'.$thumbnail.'"';
            $thumbimg .= ($alt) ? ' alt="'.$alt.'"' : '';
            $thumbimg .= ($width) ? ' width="'.$width.'"' : '';
           // $thumbimg .= ($imgattributes) ? ' '.$imgattributes : '';
        ...
        if ($call == true){ //call for price flag
            ...
            if($price2 && $price2 != '0.00'){
               ...
               // $oldprice           = '<span class="ip-slashprice12">'.$before_curr.$p2format.$after_curr.'</span>';
				  $oldprice           = $before_curr.$p2format.$after_curr;
                $oldprice           .= ($newline) ? '<br />' : ' ';
    ...
    public static function buildAgentprofile($agent_name)
    {
        if (!$agent_name) return '';
        $db     = JFactory::getDbo();
        $where  = array ('a.live_profile = '.$db->quote($agent_name));
        $query  = IpropertyHelperQuery::buildAgentsProfileQuery($db, $where);
        $db->setQuery($query, 0, 1);
        return $db->loadObject();
    }
    ...
    public static function checkImg($propid)
    {
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->select('propid')
