...
var ipMapFunctions = (function(app) {
    ...
    app.buildResults = function(data){  
        ...
        //alert(searchOptions.agent_type);
    ...
    function buildResultRowTable(index, el)
    {
       ...
       /* <div class="tooltip fade top" style="top: 230px; left: 285.483px; display: block;">
    <div class="tooltip-arrow"></div>
    <div class="tooltip-inner">
    User Notes ::
    <img class="img-polaroid ip-overview-thumb" alt="1008 Wallstreet Annexe near Gujarat college" src="http://www.raindropsinfotech.com/example/usmetrorealty//media/com_iproperty/pictures/9/180AveOfTheAmericasPhaNewYorkNYUSA362404_001_S57878d880bd47_thumb.jpg">
    <br>
    </div>
</div>*/
        ...
        if (el.id){
            if(searchOptions.agent_type == 1){
            } else {
                var imgpopover = jQuery('<a href="#saveModal" id="pro_id_'+el.id+'" data-value="'+el.id+'" class="btn btn-small hasTooltip btn-fade pro_id" data-toggle="modal" role="button" onclick="changeModalPropId(this.id);"><span id="ip_previewimg1'+index+'" class="icon-save fa fa-save"></span></a><input type="hidden" name="id" value='+el.id+'><script>function changeModalPropId(curPropRow){ console.log(\'curPropRow\'+curPropRow); console.log(jQuery(\'#\'+curPropRow).attr(\'data-value\')); jQuery(\'#modal_prop_id\').val(jQuery(\'#\'+curPropRow).attr(\'data-value\')); }</script>');
                jQuery(imgpopover).attr('data-content', img);
                 btnGroup.append(imgpopover);
                 jQuery('#ipsaveProperty').append('<input type="hidden" name="id" value='+el.id+'>');
            }
    ...
    if(listing.stype == 1){ listing.stype = 'For Sale'; }
        ...
        //alert(abc);
        var contentString ='<div class="row-fluid ip-bubble-window">' +
                                '<div class="ip-overview-img"><div class="ip-property-thumb-holder"><div class="span6"><a href="'+listing.proplink+'">'+listing.thumb+'</a></div><div class="span6"><strong>Sale Type: </strong><span>'
                                +listing.stype+'</span><br/><strong>Yearbuilt: </strong><span>'+listing.yearbuilt+
                                '</span><br/><strong>Price: </strong>'+listing.formattedprice+
                                '<br/><strong>City: </strong><span>'+listing.city
                                +'</span><br/><strong>State: </strong><span>'+listing.statename+'</span><br/></div></div></div>'
                               ;
        //custom- end                    
