...
if($this->params->get('subdivision'))$predefines     .= '<li>'.JText::_('COM_IPROPERTY_SUBDIVISION').'<span class="divider">/</span></li>';
// location
	...
	if ( $this->params->get('qs_show_subdivision', $this->settings->qs_show_subdivision) && !$this->params->get('subdivision') ){
		...
		$('#ip-qs-subdivision').jCombo(url+'&loctype=subdivision&pdvals=".$currvals."&id=', {
			...
			selected_value: '".addslashes($this->state->get('filter.subdivision'))."' 
                ...
                if ($this->settings->qs_show_cascade) $parent_field   = '#ip-qs-subdivision';	
	}	
$jcomboscript .= "
})";
		
$document->addScriptDeclaration($jcomboscript11);
...
<?php endif; ?>
                ...
                <!--  kalim added this control here -->
                    <?php if ( $this->state->get('filter.subdivision')): ?>
                         <div class="control-group center">
                            <?php echo '<h3 class="ip-property-header">'.($this->state->get('filter.subdivision')).' Community'.'</span>'; ?>
                            <?php echo '<h4 class="ip-property-header">'."<a href=\"javascript:history.go(-1)\">GO-BACK...</a>".'</span>'; ?>
                            
                        </div>
                    <?php else: ?>
                         <div class="control-group center">
                            <?php echo '<h3 class="ip-property-header">'.JText::_('Area Properties').'</span>'; ?>
                        </div>
                    <?php endif; ?>
...
<div id="ip-searchfilter-wrapper" class="well hidden-phone">
    <div id="ip-mainfilter-container">        
        <form action="index.php?option=com_iproperty&amp;view=allproperties&amp;id=0&amp;Itemid=323" method="post" name="ip_quick_search" class="ip-quicksearch-form form-inline" id="ip-quicksearch-form" novalidate="novalidate">
        <!-- <form action="index.php?option=com_iproperty&amp;view=allproperties&amp;id=0&amp;Itemid=323" method="post" name="ip_quick_search" class="ip-quicksearch-form form-inline adv-search" id="ip-quicksearch-form" novalidate="novalidate"> -->
            ...
            <div class="ip-quicksearch-optholder">
                ...
                <div class="control-group center">
                    <?php if ( $this->params->get('qs_show_keyword', $this->settings->qs_show_keyword) ): ?>
                         <div class="control-group center"><b>



                        <input type="text" class="input-medium ip-qssearch custom-width" placeholder="MLS #, Address, City & State, Zip or Subdivision" name="filter_keyword" id="filter_keyword" value="<?php echo $this->state->get('filter.keyword'); ?>" />
                        </div>
                    <?php endif; ?>
                </div>
                ...
                <div class="row-fluid">
                   ...
                   <!--  <?php if ( $this->params->get('qs_show_keyword', $this->settings->qs_show_keyword) ): ?>
                    ...
                    <?php if ( $this->params->get('qs_show_subdivision', $this->settings->qs_show_subdivision) && !$this->params->get('subdivision') ): ?>
                         <div class="span3">
                        <select name="filter_subdivision" class="input-fullwidth" id="ip-qs-subdivision"></select>
                        </div>
                    <?php endif; ?>
            ...
            <div class="ip-quicksearch-sortholder">
               ...
               <!--   moved here from all the way on bottom of this page - kalim -->
               <?php require(JPATH_COMPONENT.'/views/allproperties/tmpl/default_gmap.php'); ?>
