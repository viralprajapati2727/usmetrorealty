...
//echo $this->loadTemplate('toolbar');  // Commented By Webicules on 25 JUNE 2018 If client says then we will uncomment
...
if($this->mapOk)
{
    ...
    $cat_icon 			= IPropertyHTML::getCatMapIcon($this->p->available_cats[0]);    
    $map_house_icon     = $cat_icon ?: JURI::root(true).'/components/com_iproperty/assets/images/map/icon56.png';
...
<style>
#calcModal{ left:50% !important; margin-left:-300px !important; padding-top:15px !important; padding-right:15px !important;}
#calcModal .modal-footer{ margin-left:15px;}
.tab-content{
    background: #ffffff;
    padding: 10px;
...
#saveModal{ left:50% !important; margin-left:-300px !important; padding-top:15px !important; padding-right:15px !important;}
#saveModal .modal-footer{ margin-left:15px;}
.tab-content{
    background: #ffffff;
    padding: 10px;
    border: #dddddd 1px solid;
...
.span12.pull-left.ip-request-wrapper {
    border: none !important;
   
}
.span12.pull-left.ip_details_wrapper {
    background: none;
    border-radius: 5px;
}
.wtplwell{ font-size:10px; font-style:italic; padding-left:25px; text-align:center;}
.wtpl-tab-head{ font-size:17px; color:#1b0d7f; line-height:25px; }

.nav.nav-tabs#ipMapTabs{ margin-bottom:0 !important;}
.tab-content#ipMapContent { margin-top: -1px; overflow:hidden;}

.ip-sidecol.ip-mainaddress.span12{ padding-left:0 !important;}

@media(max-width:600px){
	#calcModal {    top: 10% !important;     left: 50%;    margin-left: -160px !important;}
...
<article class="item-page<?php echo $this->pageclass_sfx?>">
    <div class="ip-mainheader">
        <h1 class="pull-left">
            <?php echo $this->escape($this->iptitle); ?> <small class="ip-detail-price"><?php if($this->p->call_for_price==0){ if($this->p->price2==0){ echo $this->p->formattedprice;  } else {echo '$'.number_format($this->p->price2,0);} } else { echo $this->p->formattedprice; } ?></small>
        </h1>
    </div>
                    ...
                    if ($this->docs): 
						...
						foreach ($this->docs as $d){
							...
							if($doc_title && !$d->remote) {
								echo '<li class="ip_sidecol_item"><a href="'.JURI::base().$d->path.$d->fname.$d->type.'" target="_blank">' . $doc_title . ' - <b>[</b>'.substr($d->type,1).'<b>]</b></a></li>'; // [[CUSTOM]] RI , for image path changes
