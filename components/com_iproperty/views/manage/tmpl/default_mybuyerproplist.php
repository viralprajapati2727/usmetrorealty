    ...
    <tbody>
        ...
        if ($this->items)
        {
            ...
            foreach($this->items as $p)
            {
                ...
                $property_notes = ($p->notes || $thumbnail) ? ' class="" title="'.JText::_('COM_IPROPERTY_USER_NOTES').' :: '.$thumbnail.$p->notes.'"' : '';                            
                ...
                if( $p->state == 1 && $p->approved ): //property still exists
                    ...
                    echo '
                            <td class="nowrap center"><a class="btn btn-small btn-danger ipsave_delete" href="index.php?option=com_iproperty&view=manage&layout=mybuyerproplist&task=proplist.deleteSaved&id='.$p->id.'"><i class="icon-trash"></i></a></td>
                         </tr>';
                    ...
                    echo '<tr class="ip-row'.$k.'" id="'.$p->save_id.'" >
                            ...
                            <td id="'.$p->save_id.'" class="nowrap center"><a class="btn btn-small btn-danger ipsave_delete" href="index.php?option=com_iproperty&view=manage&layout=mybuyerproplist&task=proplist.deleteSaved&id='.$p->id.'""><i class="icon-trash"></i></a></td>
                         </tr>';
                endif;
                $k = 1 - $k;
            }
