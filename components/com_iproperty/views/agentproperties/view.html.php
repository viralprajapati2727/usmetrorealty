...
class IpropertyViewAgentProperties extends JViewLegacy
{
    ...
    public function display($tpl = null)
	{
            if(!$_REQUEST['id']){
                ...
                $agent_name = JRequest::getvar(agentname); //die();
                $model=$this->getModel('AgentProperties');
                $AgentId = $model->getAgentId($agent_name);
                $_REQUEST['id'] = $AgentId;  
            }
        ...
        $model=$this->getModel('AgentProperties');
        $this->testimonials = $model->testimonials();

        require_once JPATH_COMPONENT.'/models/agents.php';
        $model = new IpropertyModelAgents;
        $this->video = $model->getVideo();
      


        if(JRequest::getVar('layout','') == 'agentprofile'){
            $this->agentProfile        = IpropertyHTML::buildAgentprofile($app->input->get('agentname'));
            //echo "<pre>"; print_r($this->agentProfile); exit;
            parent::display('agentprofile');
        } else {
