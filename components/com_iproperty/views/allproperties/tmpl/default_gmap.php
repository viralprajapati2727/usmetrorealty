...
//echo "<pre>"; print_r($item);
	...
	$cat_item = $cats[$item->cat_id]; //print_r($cat_item); exit;
	...
	if($item->lat_pos && $item->long_pos){
		if($item->call_for_price==0){ 
			if($item->price2==0){ 
				$formattedprice2 =  $item->formattedprice;  
			} else {
				$formattedprice2 =  '$'.number_format($item->price2,0);
			} 
		} else { 
			$formattedprice2 =   $item->formattedprice; 
		} 
		...
		$map_js .= 'locations.push(['.$item->lat_pos.','.$item->long_pos.','.$item->id.',"'.$icon.'","'.$item->proplink.'","'.$imagePro.'",'.$item->stype.',"'.$item->beds.'","'.$formattedprice2.'","'.$item->statename.'","'.$item->street_address.'","'.$item->sqft.'","'.$item->baths.'","'.$item->cat_id.'","'.$cat_item['title'].'" ]);';	
	}
}
//$_SERVER['REMOTE_ADDR']
$ip = '68.178.213.203';
...
//echo $this->settings->adv_default_zoom;
$map_js .='
jQuery(function($) {
        ...
        if(locations.length){
			...
			$.each(locations, function(i, el){
		        ...
		        var contentString =\'<div class="row-fluid ip-bubble-window wtplmap">\' +
                    \'<div class="ip-overview-img 123 webi-mapbox"><div class="ip-property-thumb-holder"><div class="span6"><a href="\'+el[4]+\'"><img src=\'+el[5]+\'></a></div><div class="span6"><span><strong>\'
                    +el[14]+\'</strong></span><strong>Price: </strong><strong>\'+el[8]+
				...
				if((el[13] == 3) || (el[13] == 4) || (el[13] == 5) || (el[13] == 6)){
		        var contentString =\'<div class="row-fluid ip-bubble-window wtplmap">\' +
                    \'<div class="ip-overview-img 123 webi-mapbox"><div class="ip-property-thumb-holder"><div class="span6"><a href="\'+el[4]+\'"><img src=\'+el[5]+\'></a></div><div class="span6"><span><strong>\'
                    +el[14]+\'</strong></span><strong>Price: </strong><strong>\'+el[8]+
                    \'</strong><br/><strong>Sqft: </strong><span> \'+el[11]
                    +\'</span><br/><strong>Address: </strong><span> \'+el[10]+\' \'+el[9]+\'</span><br/></div></div></div>\';
				}
