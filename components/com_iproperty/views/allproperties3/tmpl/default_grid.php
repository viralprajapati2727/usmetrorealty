...
//echo 'test-kalim added this test to find out the subdivision name';
//echo ($this->state->get('filter.subdivision'));
//echo "<pre>"; print_r($this->state); exit;

      //kalim added this control here --- USE THIS IF WANT TO PUT LABEL between map and properties
      //if ($this->state->get('filter.subdivision'))
      //{
      // echo '<h2 class="ip-property-header">'.($this->state->get('filter.subdivision')).' '.JText::_('COM_IPROPERTY_PROPERTIES').'</h2><span class="pull-right small ip-pagination-results">'.$this->pagination->getResultsCounter().'</span>';
      //}else {
      ...
      //}
      //added below --this is part of the above if used
      echo '<span class="pull-right small ip-pagination-results">'.$this->pagination->getResultsCounter().'</span>';
