...
{    
    ...
    protected function getListQuery($featured = false)
	{
		...
		/*$query  = "SELECT p.id AS id, p.mls_id, p.yearbuilt, p.price, p.call_for_price, p.show_map, p.hide_address, CAST(p.price2 AS SIGNED) as price2, p.stype_freq, p.title AS title, p.beds, p.baths, p.sqft, p.lot_acres, p.lotsize, p.street_num, p.street, p.street2, p.apt, p.description as description, p.short_description, p.created, p.created_by, p.modified, p.stype, p.listing_office, p.city, p.locstate, p.county, p.region, p.subdivision, p.country, p.province, p.alias as alias, p.featured, p.latitude, p.longitude, p.available, p.expired, p.income, p.tax, p.publish_up, p.state, p.checked_out, p.checked_out_time, p.hits, p.listing_info, (SELECT DISTINCT cat_id FROM c4aqr_iproperty_propmid as pm WHERE pm.prop_id = p.id) AS cat_id FROM c4aqr_iproperty AS p LEFT JOIN c4aqr_iproperty_cities as ct ON ct.id = p.city INNER JOIN c4aqr_iproperty_countries as cn ON cn.id = p.country INNER JOIN c4aqr_iproperty_states as st ON st.id = p.locstate WHERE (STR_TO_DATE(`p`.`expired`,'%Y-%m-%d') > STR_TO_DATE('".date('Y-m-d')."','%Y-%m-%d') OR `p`.`expired` IS NULL) AND p.access IN (1,1,5) AND (p.publish_up = '0000-00-00 00:00:00' OR p.publish_up <= '".date('Y-m-d hh:mm:ss')."') AND (p.publish_down = '0000-00-00 00:00:00' OR p.publish_down >= '".date('Y-m-d hh:mm:ss')."') AND p.state = 1 AND p.approved = 1 AND (SELECT DISTINCT cat_id FROM c4aqr_iproperty_propmid as pm WHERE pm.prop_id = p.id LIMIT 1) IN (1,2,3,4,5,6) ORDER BY p.price DESC";*/ 
        //echo $query->__toString();//exit;
		//echo $query; exit;
    ...
    protected function getWhere()
    {
        ...
        //$where['searchfields']  = array('p.street_num','p.street','p.street2','p.city','p.county','p.locstate','p.subdivision','p.postcode','p.short_description','p.description','p.mls_id','cn.title','ct.title','st.title');
		
		
		$where['searchfields']  = array('p.street_num','p.city','p.county','p.locstate','p.postcode','p.short_description','p.description','p.mls_id','cn.title','ct.title','st.title','p.subdivision');
    ...
    public function getPropertiesMarkers(){
        ...
        $id = $user->id;
        $state = '';
        $myproperties = JRequest::getInt('myproperties');
        //var_dump($id);
        //var_dump($myproperties);
        if($id && $myproperties){

            $db = JFactory::getDbo();
            
            $selectagent = $db->getQuery(true);
            $selectagent->select($db->quoteName(array('locstate')));
            $selectagent->from($db->quoteName('#__iproperty_agents'));
            $selectagent->where($db->quoteName('state') . ' = 1 AND '.$db->quoteName('user_id').' = '.$db->quote($id));
            $db->setQuery($selectagent);
            //echo $query->__toString();
            $selectAgentObj = $db->loadObject();
            $state = $selectAgentObj->locstate;
            //var_dump($state);exit;
        ...
        if(!$state){
            ...
            $query->select($db->quoteName(array('id', 'street_num', 'street', 'street2', 'latitude', 'longitude')));
            $query->from($db->quoteName('#__iproperty'));
            $query->where($db->quoteName('longitude') . ' <> "" AND '. $db->quoteName('latitude').' <> "" AND '. $db->quoteName('access') . ' = 1 AND ' .$db->quoteName('approved') . ' = 1 AND '.$db->quoteName('locstate').' = '.$db->quote($state));
            $query->order('id ASC');
            $db->setQuery($query);
            //echo $query->__toString();
            $markerpros = $db->loadObjectList();
     ...
     public function focusState(){
        ...
        $db = JFactory::getDbo();
        $query = $db->getQuery(true);
        $query->select($db->quoteName(array('id', 'locstate', 'user_id')));
        ...
        $get_state_name = ipropertyHTML::getstatename($results->locstate);
        //echo "<pre>"; print_r($get_state_name); exit;
