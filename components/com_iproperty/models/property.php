...
class IpropertyModelProperty extends JModelForm
{
	...
	public function sendTofriend($post)
    {
		...
		$recipients1 = '';
		foreach($agents as $a){
			if($a->email) $recipients1 = $a->email;
		}
		// set email vars
		$recipients     = explode(',', $post['recipient_email']);
		/* PUSH Email INTO ARRAY() */
		array_push($recipients,$recipients1);	
  ...
  public function customItem($pk = null)
    {
        $pk = (!empty($pk)) ? $pk : (int) $this->getState('property.id');
        if (!$pk) return false;
        ...
        if (!isset($this->_item[$pk])) {
            ...
            $data[0]->city = ipropertyHTML::getCityName($data[0]->city);                
            $this->_item[$pk] = $data[0];
        }
        return $this->_item[$pk];exit;
    } 
}//end class
