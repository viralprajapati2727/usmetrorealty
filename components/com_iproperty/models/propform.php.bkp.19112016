...
class IpropertyModelPropForm extends IpropertyModelProperty
{
    ...
    {  
        ...
        if($propid && $doApprove && !$settings->auto_publish && !$canApprove){
            ...
            if($results->agent_type==1){
                 $db = JFactory::getDbo();
                $query = 'UPDATE #__iproperty SET approved = 1 WHERE id = '.(int)$propid.' LIMIT 1';
                $db->setQuery($query);
                return true;
        ...
        if($doApprove && $settings->notify_newprop && !$ipauth->getAdmin()){ 
        // new property - send admin a notification that a listing has been added
