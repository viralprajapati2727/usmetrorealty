...
class IpropertyControllerAjax extends JControllerLegacy
{
    ...
    public function checkAgentMLS()
    {
      //  JSession::checkToken() or die( 'Invalid Token');

        $app        = JFactory::getApplication();
        $mls_id      = $app->input->getString('mls_id');
        //$agent_id   = $app->input->getInt('agent_id', 0);
        ...
        $query->where('mls_id = '.$db->Quote($mls_id));
        $db->setQuery($query);
        //echo $query; exit;

        echo $db->loadResult();
    }
    public function checkProfileName()
    {
        $app        = JFactory::getApplication();
        $live_profile      = $app->input->getString('live_profile');
        //$agent_id   = $app->input->getInt('agent_id', 0);
        if(!$live_profile) return false;

        $db = JFactory::getDbo();
        $query = $db->getQuery(true);

        $query->select('live_profile');
        $query->from('#__iproperty_agents');
        $query->where('live_profile = '.$db->Quote($live_profile));
        $db->setQuery($query);
        //echo $query; exit;
