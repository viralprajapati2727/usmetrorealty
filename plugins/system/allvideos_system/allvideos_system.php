 ...
 * @author    	Raindrops Infotech - raindropsinfotech.com
 */
...
class plgSystemAllvideos_system extends JPlugin {

	// reference parameters
	var $plg_name					= "allvideos_system";
	var $plg_copyrights_start		= "\n\n<!-- JoomlaWorks \"AllVideos\" Plugin (v4.6.1) starts here -->\n";
	var $plg_copyrights_end			= "\n<!-- JoomlaWorks \"AllVideos\" Plugin (v4.6.1) ends here -->\n\n";

	function plgSystemAllvideos_system( &$subject, $params ) {
	...
	function onAfterRender(){
		...
		if($app->isSite()){
			...
			$result = $this->renderAllVideos($row, $params, $page = 0);
	...
	function renderAllVideos(&$row, &$params, $page = 0) {
		...
		if (version_compare(JVERSION, '1.6.0', 'ge')) {
			$pluginLivePath = $siteUrl.'/plugins/system/'.$this->plg_name.'/'.$this->plg_name;
		} else {
		...
		if (preg_match("#{(".$grabTags.")}#is", $row)==false) return; //$row->text
		...
		foreach ($tagReplace as $plg_tag => $value) {
			...
			// process tags
			if (preg_match_all($regex, $row, $matches, PREG_PATTERN_ORDER)) { //$row->text
				...
				foreach ($matches[0] as $key => $match) {
					...
					// Output
					$row = preg_replace("#{".$plg_tag."}".preg_quote($tagcontent)."{/".$plg_tag."}#is", $getTemplate , $row); //$row->text , $row->text
