...
class RequestCore
{
	...
	public function prep_request()
	{
        ...
        $params = JComponentHelper::getParams( 'com_jaamazons3' );
        $enableProxy = $params->get('proxy_enable');
        if($enableProxy)
        {
            $proxyHost = $params->get('proxy_host');
            $proxyPort = $params->get('proxy_port');
            $proxyUsername = $params->get('proxy_username');
            $proxyPassword = $params->get('proxy_password');
            if(!empty($proxyHost) AND !empty($proxyPort) AND !empty($proxyUsername) AND !empty($proxyPassword))
            {
                $proxy = array(
                    'host' => $proxyHost,
                    'port' => $proxyPort,
