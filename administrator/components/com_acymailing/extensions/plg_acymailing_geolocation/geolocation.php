	 ...
	 function acymailinggeolocation_show(){?>
	 	<script language="javascript" type="text/javascript">
			function applyTag(tagname){
				...
				if (document.adminForm.geolocType.value != '0'){ string += '|info:'+document.adminForm.geolocType.value; }
				string += '}';
				setTag(string);
				insertTag();
			}
		...
		foreach($fields as $fieldname => $oneField){
			...
			$text .= '<tr style="cursor:pointer" class="row'.$k.'" onclick="applyTag(\''.$fieldname.$type.'\');" ><td class="acytdcheckbox"></td><td>'.$fieldname.'</td><td>'.@$descriptions[$fieldname].'</td></tr>';
			$k = 1-$k;
		}
