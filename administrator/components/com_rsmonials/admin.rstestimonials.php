...
<fieldset class="adminform">
	...
	<?php
	 ...
	 if(fetchParam('show_single_name_field') == 'true') { ?>
	 	<tr>
	 		<td class="key">Agents Name:<span style="color:red;">*</span></td>
	 		<td colspan="2">
	 			<select id="agent_id" name="agent_id" aria-invalid="false">
...
function save() {
	...
	if($_POST['id']>0) {
		$database->setQuery("update `#__".RSWEBSOLS_TABLE_PREFIX."` set `fname`='".$database->escape($_POST['fname'])."', `lname`='".$database->escape($_POST['lname'])."', `about`='".$database->escape($_POST['about'])."', `location`='".$database->escape($_POST['location'])."', `website`='".$database->escape($_POST['website'])."', `email`='".$database->escape($_POST['email'])."', `comment`='".$database->escape($_POST['comments'])."', `agent_id`='".$database->escape($_POST['agent_id'])."', `date`='".$date."' where `id`='".$_POST['id']."'");
	}
	else {
		$database->setQuery("insert into `#__".RSWEBSOLS_TABLE_PREFIX."`(`id`, `fname`, `lname`, `about`, `location`, `website`, `email`, `comment`, `agent_id` , `ip`, `date`, `status`) values('', '".$database->escape($_POST['fname'])."', '".$database->escape($_POST['lname'])."', '".$database->escape($_POST['about'])."', '".$database->escape($_POST['location'])."', '".$database->escape($_POST['website'])."', '".$database->escape($_POST['email'])."', '".$database->escape($_POST['comments'])."', '".$database->escape($_POST['agent_id'])."', '".getClientIP()."', '".$date."', '1')");
	}
	$database->query();
