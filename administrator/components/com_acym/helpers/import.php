...
class acymimportHelper extends acymObject
{
    ...
    public function cms()
    {
        ...
        $time = time();
        $query = 'INSERT IGNORE INTO #__acym_user (`name`,`email`,`creation_date`,`active`,`cms_id`, `source`) SELECT `user`.`'.$this->cmsUserVars->name.'`,`user`.`'.$this->cmsUserVars->email.'`,`user`.`'.$this->cmsUserVars->registered.'`,1 - `user`.'.$this->cmsUserVars->blocked.',`user`.`'.$this->cmsUserVars->id.'`,\'Import on '.acym_date($time, 'Y-m-d').'\' FROM '.$this->cmsUserVars->table.' AS `user` ';
    ...
    public function database()
    {
        ...
        $select['`source`'] = acym_escapeDB('Import on '.acym_date($time, 'Y-m-d'));
    ...
    public function _checkData(&$user, $timestamp)
    {
        ...
        if (empty($user->source)) {
            $user->source = 'Import on '.acym_date($timestamp, 'Y-m-d');
        }
