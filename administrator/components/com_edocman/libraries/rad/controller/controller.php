...
class OSController
{
	 ...
	 * @return OSController
	 */
	public static function getInstance($option, OSInput $input = null, array $config = array())
	{
		...
		{
			$name = OSInflector::singularize($input->get('view'));
			if (!$name)
			{
				$name = 'controller';
			}
		...
		if (!isset(self::$instances[$component . $name]))
		{
			...
			if ($view)
			{
				$class = $prefix . 'Controller' . ucfirst(OSInflector::singularize($view));
			}
			else
			{
	...
	public function __construct(OSInput $input = null, array $config = array())
	{
		...
		$xMethods = get_class_methods('OSController');
		...
		//Add tables path, make it easy to use table in controller
	 ...
	 *
	 * @return OSController return itself to support changing
	 */
	 ...
	 *
	 * @return OSController A OSController object to support chaining.
	 */
	...
	public function getModel($name = '', array $config = array())
	{
		...
		{
			if (OSInflector::isPlural($name))
			{
				$config['default_model_class'] = 'OSModelList';
			}
			else
			{
				if ($this->app->isAdmin())
				{
					$config['default_model_class'] = 'OSModelAdmin';
				}
				else
				{
					$config['default_model_class'] = 'OSModelItem';
				}
			}
		}

		//Create model and auto populate model states if required
		$model = OSModel::getInstance($name, $this->classPrefix . 'Model', $config);
	 ...
	 *
	 * @return OSView Reference to the view
	 *
	 */
	public function getView($name, $type = 'html', $layout = 'default', array $config = array())
	{
		...
		if (!isset($config['paths']))
		{
			...
			$paths[]         = JPATH_BASE . '/components/' . $this->option . '/view/' . $name . '/tmpl';			
			$config['paths'] = $paths;
		}
		...
		if (!isset($config['default_view_class']))
		{
			if (OSInflector::isPlural($name))
			{
				$config['default_view_class'] = 'OSViewList';
			}
			else
			{
				$config['default_view_class'] = 'OSViewItem';
			}
		}
		if ($this->app->isAdmin())
		{
		...
		return OSView::getInstance($name, $type, $this->classPrefix . 'View', $config);
	}
	 ...
	 *
	 * @return OSController This object to support chaining.
	 *
	 */
	 ...
	 *
	 * @return OSInput The input object.
	 *
	 */
