<?php
        ...
        $date = date('Y-m-d')."+";
        $date2 = date('h:i:s');
        echo "AZ Images starting at:"; echo $date2;
        ...
        if (mysqli_num_rows($result) > 0) {
            ...
            while($row = mysqli_fetch_assoc($result)){
                ...
                if(!empty($row['int_list_id'])){
                    $photos = $rets->GetObject("Property", "640x480", $row['int_list_id'], "*", 1);
                } else {
                ...
                if(empty($check_result)){
                    ...
                    //echo " <photos count more then 0>"; print_r(count($photos));
                    $ordering = 0;
                    $my_values = array();
                    $insert_photos = 0;
        ...
        if($count > 0){
            ...
            $sql = "SELECT offset_number FROM set_images_offset WHERE id = 1 ";
                $result = mysqli_query($conn, $sql);
                ...
                $sql = "UPDATE set_images_offset SET offset_number = ".($offset+$count)." WHERE id=1 "; //Kalim Added this to AZ props - do not know if needed
                $resultQ2 = mysqli_query($conn, $sql);//Kalim Added this to AZ props - do not know if needed
